    ...
    public class GameMatchingResponseProcessor : PacketProcessor
    {
        public override void Process(PacketManager packetmanager, byte[] data)
        {
            ...
            for (int i = 0; i < Data.PlayersLength; ++i)
            {
                ...
                Define.PlayerData pData = new();
                pData.id = id;
                pData.team = team;
                pData.pos = pos;
                pData.nickname = nickname;

                Managers.Game.AddPlayerData(id, pData);

                Debug.Log($"PlayerData info ID : {id}, TEAM : {team}, POS: ({position.X}, {position.Y}, {position.Z}), NICKNAME : {nickname}");
            }
        }
    }
}
