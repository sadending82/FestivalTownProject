...
class Packet_PlayerFlyingKick : public PacketProcessor {

public:
	Packet_PlayerFlyingKick(Server* server, PacketSender* packetSender) : PacketProcessor(server, packetSender) {}

	virtual void Process(const uint8_t* data, const int size, const int key) {
		try {
			flatbuffers::Verifier verifier(data, size);
			if (verifier.VerifyBuffer<PlayerFlyingKick>(nullptr)) {

				const PlayerFlyingKick* read = flatbuffers::GetRoot<PlayerFlyingKick>(data);
