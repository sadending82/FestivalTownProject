...
struct BombDrop FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  typedef BombDropBuilder Builder;
  enum FlatBuffersVTableOffset FLATBUFFERS_VTABLE_UNDERLYING_TYPE {
    VT_PLAYER_ID = 4,
    VT_BOMB_INFO = 6
  };
  ...
  const PacketTable::ObjectTable::BombPosition *bomb_info() const {
    return GetPointer<const PacketTable::ObjectTable::BombPosition *>(VT_BOMB_INFO);
  }
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           VerifyField<int32_t>(verifier, VT_PLAYER_ID, 4) &&
           VerifyOffset(verifier, VT_BOMB_INFO) &&
           verifier.VerifyTable(bomb_info()) &&
           verifier.EndTable();
  }
};

struct BombDropBuilder {
  typedef BombDrop Table;
  ::flatbuffers::FlatBufferBuilder &fbb_;
  ::flatbuffers::uoffset_t start_;
  void add_player_id(int32_t player_id) {
    fbb_.AddElement<int32_t>(BombDrop::VT_PLAYER_ID, player_id, 0);
  }
  void add_bomb_info(::flatbuffers::Offset<PacketTable::ObjectTable::BombPosition> bomb_info) {
    fbb_.AddOffset(BombDrop::VT_BOMB_INFO, bomb_info);
  }
  explicit BombDropBuilder(::flatbuffers::FlatBufferBuilder &_fbb)
        : fbb_(_fbb) {
    start_ = fbb_.StartTable();
  }
  ::flatbuffers::Offset<BombDrop> Finish() {
    const auto end = fbb_.EndTable(start_);
    auto o = ::flatbuffers::Offset<BombDrop>(end);
    return o;
  }
};

inline ::flatbuffers::Offset<BombDrop> CreateBombDrop(
    ::flatbuffers::FlatBufferBuilder &_fbb,
    int32_t player_id = 0,
    ::flatbuffers::Offset<PacketTable::ObjectTable::BombPosition> bomb_info = 0) {
  BombDropBuilder builder_(_fbb);
  builder_.add_bomb_info(bomb_info);
